...
async function fetchAndFilterVulnerabilityAlerts(
  ...
  while (true) {
    ...
    if (!endCursor) {
      core.warning("pageInfo.hasNextPage is true, but endCursor is missing; stopping pagination to avoid infinite loop")
      break
    }
  }
...
export async function getAlert (name: string, version: string, directory: string, client: InstanceType<typeof GitHub>, context: Context, fetchDepth: number = 0): Promise<dependencyAlert> {
